...
export function scanLibraries(dir, instance) {
    const libs = [];
    try {
...
function countMemories(file) {
    let db = null;
    try {
        db = new Database(file, { readonly: true, fileMustExist: true });
        const row = db.prepare('SELECT COUNT(*) AS n FROM memory WHERE is_active = 1').get();
