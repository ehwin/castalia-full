...
export function listCategories() {
    ...
    const rows = db.prepare('SELECT * FROM categories ORDER BY parent, name').all();
    return rows.map(r => ({
        name: r.name,
...
export function createCategory(name, description, parent, color) {
    const db = DatabaseManager.getInstance();
    ...
    if (parent) {
        ...
        if (!parentExists)
            throw new Error(`Parent category "${parent}" does not exist`);
    }
    db.prepare(`
...
export function deleteCategory(name, reassignTo, reassignChildrenTo) {
    const db = DatabaseManager.getInstance();
    const cat = db.prepare('SELECT * FROM categories WHERE name = ?').get(name);
    if (!cat)
        throw new Error(`Category "${name}" not found`);
    // Check for children
    const children = db.prepare('SELECT name FROM categories WHERE parent = ?').all(name);
    ...
    // Check for memories
    const memCount = db.prepare('SELECT COUNT(*) as cnt FROM memory WHERE category = ? AND is_active = 1').get(name);
