...
function err(msg: string, code = 'ERROR') {
  return {
    content: [{ type: 'text' as const, text: JSON.stringify({ ok: false, error: { code, message: msg } }, null, 2) }],
    isError: true,
  };
}
...
server.tool(
  'memory_search_all',
  'Federation search (read-only): search across every configured member — same-machine library dirs (local-dir) and remote MCP endpoints (mcp-http). Members are declared by FEDERATION_MEMBERS or legacy FEDERATION_DIRS. Explicit semantics: results are not merged with any local engine logic; the caller decides interop policy. Each hit carries instance/member/project. Per-member failures are reported in `errors` instead of being silently dropped.',
  {
...
server.tool(
  'federation_members',
  'List federation members with their transport, variant, declared capabilities and size (libraries / memories). Use it to see who can be collected and what each member offers (e.g. capabilities: ["emotion"]).',
  {},
  async () => {
    try {
