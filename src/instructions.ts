...
export type InstructionScope = 'global' | 'user' | 'project';
 ...
 * L3 仅在传入 project 时查询。
 */
export function getInstruction(project?: string | null): Instruction[] {
  const db = DatabaseManager.getInstance();
  const result: Instruction[] = [];
  const l1 = db.prepare('SELECT scope, project, content FROM instructions WHERE scope = ?').get('global') as any;
  const l2 = db.prepare('SELECT scope, project, content FROM instructions WHERE scope = ?').get('user') as any;
  if (l1) result.push({ scope: l1.scope, project: l1.project, content: l1.content });
  if (l2) result.push({ scope: l2.scope, project: l2.project, content: l2.content });
  const proj = (project ?? '').trim();
  if (proj.length > 0) {
    const l3 = db.prepare('SELECT scope, project, content FROM instructions WHERE scope = ? AND project = ?').get('project', proj) as any;
    if (l3) result.push({ scope: l3.scope, project: l3.project, content: l3.content });
  }
  return result;
}
