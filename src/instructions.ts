...
export type InstructionScope = 'global' | 'user' | 'project' | 'rule';
...
export interface InstructionWithMeta extends Instruction {
  id: string;
  updatedAt: string;
}
 ...
 */
function dbForScope(scope: InstructionScope, project: string | null): any {
  if (scope === 'project') {
...
export function ensureSeedInstructions(): { seeded: boolean; count: number } {
  const db = DatabaseManager.getGlobal();
  const row = db.prepare('SELECT COUNT(*) as c FROM instructions').get() as any;
...
export function saveInstruction(
  ...
  const existing = db.prepare('SELECT id FROM instructions WHERE scope = ? AND project IS ?').get(scope, project) as any;
...
export function parsePaths(raw: string | null | undefined): string[] | null {
  if (!raw) return null;
  try {
    const v = JSON.parse(raw);
    if (Array.isArray(v)) {
      const arr = v.filter((x: any) => typeof x === 'string');
 ...
 */
export function matchPaths(paths: string[] | null | undefined, path?: string): boolean {
  if (!paths || paths.length === 0) return true;
  if (path === undefined || path === null || path === '') return true;
  const p = path.replace(/\\/g, '/');
  let allowed = false;
  for (const raw of paths) {
    if (raw.startsWith('!')) {
      if (getMatcher(raw.slice(1))(p)) allowed = false;
...
export function parseIncludeRefs(raw: string): string[] {
  const t = raw.trim();
  try {
...
export function getRuleGroup(name: string): { content: string; paths: string[] | null } | null {
  const db = DatabaseManager.getGlobal();
  const row = db.prepare('SELECT content, paths FROM instructions WHERE scope = ? AND project = ?').get('rule', name) as any;
  if (!row) return null;
 ...
 */
export function expandInstruction(content: string, depth = 0, seen = new Set<string>(), path?: string): string {
  const out: string[] = [];
  for (const line of content.split('\n')) {
    ...
    for (const ref of parseIncludeRefs(m[1])) {
      ...
      if (!rule) { out.push(`⚠️ [include] 未找到规则组 "${name}"`); continue; }
      if (seen.has(name)) { out.push(`⚠️ [include] 环路引用已截断:"${name}"(出现在当前展开链)`); continue; }
      if (depth >= MAX_INCLUDE_DEPTH) { out.push(`⚠️ [include] 超过 ${MAX_INCLUDE_DEPTH} 层深度上限,"${name}" 已截断`); continue; }
      // 规则组自身带 paths 时也受调用方 path 过滤(为空则跟随引用方)
      if (rule.paths && rule.paths.length && path !== undefined && !matchPaths(rule.paths, path)) continue;
 ...
 */
export function getInstruction(project?: string | null, path?: string): Instruction[] {
  const gdb = DatabaseManager.getGlobal();
  const result: Instruction[] = [];
  const cands: any[] = [];
  const l1 = gdb.prepare('SELECT scope, project, content, paths FROM instructions WHERE scope = ?').get('global') as any;
  const l2 = gdb.prepare('SELECT scope, project, content, paths FROM instructions WHERE scope = ?').get('user') as any;
  if (l1) cands.push(l1);
  if (l2) cands.push(l2);
  ...
  if (proj.length > 0) {
    ...
    const l3 = pdb.prepare('SELECT scope, project, content, paths FROM instructions WHERE scope = ? AND project = ?').get('project', proj) as any;
    if (l3) cands.push(l3);
  }
  for (const row of cands) {
    if (!matchPaths(parsePaths(row.paths), path)) continue;
    result.push({
      scope: row.scope,
      project: row.project,
...
export function listInstructions(): InstructionWithMeta[] {
  const rows: any[] = [];
  const gdb = DatabaseManager.getGlobal();
  ...
  rows.sort((a, b) => {
    const rank = (s: string) => (s === 'global' ? 1 : s === 'user' ? 2 : 3);
...
export function deleteInstruction(scope: InstructionScope, project?: string | null): { deleted: boolean } {
