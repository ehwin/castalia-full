...
const buffers = new Map<string, SessionMemoryBuffer>();
...
export class SessionMemoryBuffer {
  ...
  public tokenThreshold: number;
  public pendingCount = 0;
  public pendingTokens = 0;
  public isExtracting = false;

  private history: BufferMessage[] = [];

  constructor(project: string, sessionId: string, characterId: string = CHAR_ID, threshold: number = DEFAULT_BUFFER_SIZE, tokenThreshold: number = DEFAULT_BUFFER_TOKENS) {
  ...
  onNewMessage(messages: BufferMessage[]): void {
   ...
   */
  forceFlush(messages: BufferMessage[] = []): void {
    if (this.pendingCount > 0) {
      this.flush(messages);
    }
  }

  flush(messages: BufferMessage[]): void {
    if (this.isExtracting) return; // 互斥锁:后台已有提取任务,本次放弃(下轮再触发)
    this.pendingCount = 0;
...
export function getBuffer(project: string, sessionId: string, characterId?: string): SessionMemoryBuffer {
...
export function flushAllBuffers(): void {
  for (const b of buffers.values()) {
    try { b.forceFlush(); } catch (e: any) { console.error('[buffer] flushAll error:', e.message); }
  }
}
